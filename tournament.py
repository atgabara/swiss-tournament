...
def playerStandings():
    ...
    conn = connect()
    c = conn.cursor()
    c.execute("""SELECT w.id as id, w.name as name, w.wins as wins, w.wins + l.losses as matches
                 FROM
                 (SELECT p.id as id, p.name as name, COUNT(m.id) as wins
                 FROM players p
                 LEFT JOIN matches m ON p.id = m.winner
                 GROUP BY p.id) w
                 LEFT JOIN
                 (SELECT p.id as id, p.name as name, COUNT(m.id) as losses
                 FROM players p
                 LEFT JOIN matches m ON p.id = m.loser
                 GROUP BY p.id) l on w.id = l.id
                 ORDER BY wins""")
