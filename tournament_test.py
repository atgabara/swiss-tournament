...
def testCount():
    ...
    if c != 1:
        raise ValueError(
            ("After one player registers, countPlayers() should be "
                "1. Got {c}").format(c=c))
    ...
    if c != 2:
        raise ValueError(
            ("After two players register, countPlayers() should be "
                "2. Got {c}").format(c=c))
...
def testReportMatches():
    ...
    for (i, n, w, m) in standings:
        ...
        if w != 0:
            raise ValueError("After deleting matches, players should have "
                             "zero wins recorded.")
    print("8. After match deletion, player standings are properly "
